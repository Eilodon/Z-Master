...
export default function App() {
  ...
  useEffect(() => {
    if (zenData?.action === 'emergency_protocol') {
      console.warn("[App] Emergency Protocol Triggered by AI/SafetyGuard");
      setEmergencyActive(true);
      // Optional: Disconnect audio to focus on breathing
      disconnect();
    }
  }, [zenData, disconnect, setEmergencyActive]);

  // Combined Emergency Trigger (Manual + AI)
  useEffect(() => {
    if (emergencyActive) {
  ...
  }, [emergencyActive, status, disconnect]);
  ...
  useEffect(() => {
    const updateViz = () => {
      if (!analyserRef.current && status.kind === 'processing' && inputMode === 'text') {
  ...
  return (
    <div className="relative h-[100dvh] w-full overflow-hidden bg-stone-50 select-none font-sans text-stone-900">
      ...
      {isLoading && (
        <LoadingScreen
          onStartInteraction={requestInitialPermissions} // Use centralized hook
      ...
      <div className="absolute inset-0 z-30 flex flex-col items-center pointer-events-none">
        <div className="w-full h-full pointer-events-auto overflow-y-auto no-scrollbar pt-32 pb-48 px-4 flex flex-col items-center">
          <div className="w-full max-w-lg flex flex-col gap-6 my-auto">
            {zenData ? (
              <>
                <ZenCard data={zenData} isGenerating={status.kind === 'processing' || status.kind === 'speaking' || connectionState === 'reconnecting'} />
