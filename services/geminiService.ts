...
export const sendZenTextQuery = async (
  ...
  // 1. Safety Guardrail Scan (Pre-check)
  const safetyCheck = SafetyGuard.scanInput(text);
  if (!safetyCheck.isSafe) {
    return SafetyGuard.getMockCrisisResponse(safetyCheck.triggerWord || 'detected_harm');
  }
...
export class ZenLiveSession {
  ...
  async connect(isReconnect = false): Promise<AnalyserNode> {
    ...
    try {
      ...
      // Fail fast if permissions are missing (App should have prevented this)
